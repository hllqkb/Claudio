...
import { api, type NcmPlaylistSummary, type NcmTrack, type QueueItem } from "../api/client";
...
export default function PlaylistPage() {
  const [playlists, setPlaylists] = useState<NcmPlaylistSummary[]>([]);
  ...
  const fetchPlaylists = useCallback(async () => {
    try {
      const data = await api.getNcmPlaylists();
      setPlaylists(data.playlists);
    } catch (err) {
      console.error("Failed to fetch NCM playlists:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openPlaylist = async (pl: NcmPlaylistSummary) => {
    setSelected(pl);
    setView("detail");
  ...
  return (
    <div className="pl-page">
      {view === "list" ? (
        <PlaylistList playlists={playlists} onSelect={openPlaylist} t={t} />
      ) : (
        <PlaylistDetail
          playlist={selected!}
          tracks={tracks}
